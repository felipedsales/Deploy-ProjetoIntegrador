...
CREATE TYPE tipo_contrato_enum AS ENUM ('CLT', 'PJ', 'Freelance', 'Estágio');
CREATE TYPE modalidade_enum AS ENUM ('Presencial', 'Remoto', 'Híbrido');
CREATE TYPE status_vaga_enum AS ENUM ('Ativa', 'Inativa', 'Pausada');
CREATE TYPE status_candidatura_enum AS ENUM ('Pendente', 'Em análise', 'Aprovada', 'Rejeitada', 'Contratada');
CREATE TYPE tipo_usuario_enum AS ENUM ('candidato', 'empresa');
CREATE TYPE status_denuncia_enum AS ENUM ('Pendente', 'Em análise', 'Resolvida', 'Descartada');
...
CREATE TABLE IF NOT EXISTS vagas (
    ...
    modalidade modalidade_enum DEFAULT 'Presencial',
    localizacao VARCHAR(255),
    beneficios TEXT,
    status status_vaga_enum DEFAULT 'Ativa',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS denuncias (
    ...
    status status_denuncia_enum DEFAULT 'Pendente',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
